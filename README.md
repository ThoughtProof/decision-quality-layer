...
**Auth (non-sandbox):** `X-DQL-Key: dqlk_…` **or** `X-DQL-Account: dqla_…` (alias `Authorization: Bearer` with the same prefix). The account token bills the bound credit ledger and never returns the raw verify key. `dqla_…` is not accepted as `X-DQL-Key`. Account path: per-account reserve + fencing token + full request digest; commit must be acknowledged; optional `Idempotency-Key` / `X-Request-Id`. Expired holds are swept by `/dql/internal/sweep-reservations`. See `docs/ENV.md`.
