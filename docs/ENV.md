...
`POST /dql/verify` also accepts that same account header: `X-DQL-Account: dqla_…` or `Authorization: Bearer dqla_…`. Admission **atomically reserves** prepaid credit (or confirmed PAYG) and daily-cap **before** the engine runs (one Redis Lua EVAL). The reservation is bound to **stored key hash + Idempotency-Key + payload digest** (mandate / proposed_action / reasoning / axes / sandbox). Cross-account reuse of the same id → **403** `IDEMPOTENCY_KEY_BOUND`. Same account, different payload → **409** `IDEMPOTENCY_PAYLOAD_MISMATCH`. In-flight same id+payload → **409** `IDEMPOTENCY_IN_PROGRESS` (engine does not run again). `committed` replay returns the **stored result** (no second engine, no second debit).

**Lease / crash recovery:** credit + cap are taken at reserve. The execution lease is 15 minutes; the reservation record is kept 7 days so a stale `held` can be refunded. A later reserve/recover/sweep of that id releases an expired hold (credit + cap restored). The debit is not left on a disappearing 15-minute key. CONFIG_INVALID / thrown verify still **release** immediately.

**PAYG:** meter must succeed before commit. Meter off/failed/skipped → **503** `METER_UNAVAILABLE`, hold released, result is not stored and cannot be replayed for free. Prepaid `credit` path: debit at reserve, commit after success.
