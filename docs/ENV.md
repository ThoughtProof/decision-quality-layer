...
**Lease / crash recovery:** credit + cap are taken at reserve. The execution lease is 15 minutes; the reservation record is kept 7 days so a stale `held` can be refunded. Production recovery is `GET|POST /dql/internal/sweep-reservations` on a Vercel Cron (`*/5 * * * *` in `vercel.json`), authenticated with `Authorization: Bearer` matching `DQL_CRON_SECRET` or `CRON_SECRET`. Sweep refunds expired `held` rows without the client retrying that id. It does not refund `meter_pending`. CONFIG_INVALID / thrown verify still **release** immediately (matching fence only). Commit EVAL failure is **503** `COMMIT_UNAVAILABLE` — not a 200 and not a durable fake commit.
