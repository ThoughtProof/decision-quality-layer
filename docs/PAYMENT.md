...
Verify (`X-DQL-Key: dqlk_…`) is unchanged. An account token presented as `X-DQL-Key` is rejected. `POST /dql/verify` also accepts `X-DQL-Account: dqla_…` / `Authorization: Bearer dqla_…` and decrements the same prepaid ledger. Invalid/missing account token → **401**. Rotate/revoke still stops verify. Response body stays the native verify contract (no raw key).
