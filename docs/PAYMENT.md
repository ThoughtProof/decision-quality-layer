...
Verify (`X-DQL-Key: dqlk_…`) is unchanged. An account token presented as `X-DQL-Key` is rejected. `POST /dql/verify` also accepts `X-DQL-Account: dqla_…` / `Authorization: Bearer dqla_…`. Account path: atomic **reserve** (per-account Redis key + fencing token + full request digest) → execute once → **commit** stored result (or **release** on CONFIG_INVALID / throw). In-flight retry → **409** `IDEMPOTENCY_IN_PROGRESS`. Two accounts may share an ordinary Idempotency-Key. Same account, different digest → **409** `IDEMPOTENCY_PAYLOAD_MISMATCH`. `committed` replay returns the stored result. Stale `held` (lease 15 min; record kept 7 days) is refunded by `/dql/internal/sweep-reservations` (GitHub Actions every 15 minutes; Vercel Cron `0 6 * * *` is a Hobby-legal daily backup — do not put `*/5` in `vercel.json`). Sweep Redis failure is **503** `SWEEP_UNAVAILABLE`. PAYG meter off/fail stores `meter_pending` and returns **503** `METER_UNAVAILABLE`; retry remeters only. Commit EVAL failure → **503** `COMMIT_UNAVAILABLE`. Invalid/missing account token → **401**. Zero credits / over cap fail closed **before** the engine. Rotate/revoke still stops verify. Response body stays the native verify contract (no raw key).
