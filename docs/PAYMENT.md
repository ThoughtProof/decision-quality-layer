...
Verify (`X-DQL-Key: dqlk_…`) is unchanged. An account token presented as `X-DQL-Key` is rejected. `POST /dql/verify` also accepts `X-DQL-Account: dqla_…` / `Authorization: Bearer dqla_…`. Account path: **reserve** credit + daily-cap → `runVerification` → **commit** (or **release** on CONFIG_INVALID / throw). Invalid/missing account token → **401**. Zero credits / over cap fail closed **before** the engine. Rotate/revoke still stops verify. Response body stays the native verify contract (no raw key).
