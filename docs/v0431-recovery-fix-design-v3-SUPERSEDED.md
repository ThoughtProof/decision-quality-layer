# v0.4.3.1 CB Recovery Fix — Design Draft v3 (SUPERSEDED)

> **SUPERSEDED by v4** — Hermes-Review 2026-07-11 identifizierte zwei Blocker + drei weitere Semantik-Lücken:
> (1) caller-kontrolliertes `recoverFromOpen()` (K und Bound vom Router übergeben → Escape-Hatch),
> (2) rohe Wall-Clock im Probe (führt PR-#11-Bug wieder ein),
> (3) bestehende HALF_OPEN-Semantik falsch beschrieben,
> (4) Bound-Kalibrierung wissentlich verschoben (bekannter 15s-vs-24s-Konflikt),
> (5) `provider_route`-Enum-Erweiterung, Sampling/Recovery-Test-Vermischung, fehlende trip-generation.
> **Nicht zur Implementierung.** Historischer Zwischenstand. Siehe `docs/v0431-recovery-fix-design.md` (v4).
