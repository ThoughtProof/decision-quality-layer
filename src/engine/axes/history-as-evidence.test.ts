...
${p.user}`;
  ...
  it('reversibility prompt includes ROUTINE AUTHORIZED SCHEDULED PATH hard rule', () => {
    const p = buildReversibilityPrompt({
      ...base,
      proposed_action: JSON.stringify({
        action: 'payment.schedule',
        amount_eur: 42.9,
      ...
      context: JSON.stringify({
        evidence: {
          past_payments_to_same_iban: 11,
          amount_variance_from_history: 0.0,
          recurring_billing_pattern_confirmed: true,
        },
        history_signals: {
          past_payments_to_same_iban: 11,
          amount_variance_from_history: 0.0,
        ...
        commitment_shape: {
          action: 'payment.schedule',
          execution_window_hours: 24,
          immediate_irrevocable: false,
        },
      }),
    });
    const text = `${p.system}
${p.user}`;
    expect(text).toContain('ROUTINE AUTHORIZED SCHEDULED PATH');
    expect(text).toContain('execution_window_hours');
    expect(text).toContain('past_payments_to_same_iban');
    expect(text).toContain('payment.schedule');
    expect(text).toContain('DO return PASS on mandate-explicit routine scheduled');
    // keep consumer demo failure mode
    expect(text).toContain('binding travel booking');
    expect(text).toContain('consumer checkout');
